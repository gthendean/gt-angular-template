...
module.exports = function(grunt) {
  ...
  // Time how long tasks take for optimizing build times
  ...
  var globalCfg = {
    ...
    dist: 'dist',
    deployrepo: 'https://github.com/'   /* add deployment repository if necessary */
  };
  ...
  grunt.initConfig({
    ...
    // Watches for file change, and runs tasks based on the changed files
    watch: {
      bower: {
        files: ['bower.json'],
        tasks: ['wiredep']
    ...
    //  - dist: used for assembling final artifacts, and
    //          pushing to deployment on Git using "gh-pages" task
    clean: {
      serve: {src: '.tmp'},
      dist: {
        files: [{
    ...
    // Parse css & Add vendor prefixed styles
    autoprefixer: {
      options: {
  ...
  grunt.registerTask('default', 'List available tasks', function(){
      grunt.log.writeln(['grunt [serve | unit-tests | e2e-tests | tests | build | deploy | clean-nodes | clean-bower]']);
  });


  // Clean node_modules
  grunt.registerTask('clean-nodes', 'Clean node_modules folder', ['clean:nodeModules']);

  // Clean bower_components
  grunt.registerTask('clean-bower', 'Clean bower_components folder', ['clean:bowerComponents']);
  ...
  grunt.registerTask('serve', 'Prepare, compile, then start connection to web server', function (target) {
    ...
    // grunt serve:build - Serving from the "build" directory
    if (target === 'build') {
